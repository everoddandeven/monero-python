...
from monero import (
  ...
  MoneroWalletConfig, MoneroNetworkType, MoneroWalletFull
)
from utils import ConnectionChangeCollector, MoneroTestUtils as Utils, SampleConnectionListener
...
def test_connection_manager_demo():
  ...
  con1 = MoneroRpcConnection("https://xmr.vvv.systems")
  con1.priority = 1
  connectionManager.add_connection(con1) # use localhost as first priority
  con2 = MoneroRpcConnection("https://xmr.cryptostorm.is")
  connectionManager.add_connection(con2) # default priority is prioritized last
  
  # set current connection
  con3 = MoneroRpcConnection("https://dewitte.fiatfaucet.com")
  ...
  print(f"Best connection is online: {bestConnection.is_online()}")
  print(f"Best connection is authenticated: {bestConnection.is_authenticated()}")
  ...
  print("Connection manager cleared")


test_connection_manager_demo()